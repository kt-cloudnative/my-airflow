...
COMMON = ICISCmmn(DOMAIN='rater',ENV='dev', NAMESPACE='t-rater'
                , WORKFLOW_NAME='cdr-batch',WORKFLOW_ID='5c36ba3e3ffe4b45a86a1508aa9b233b', APP_NAME='NBSS_TRAT', CHNL_TYPE='TR', USER_ID='91337910')

with COMMON.getICISDAG({
    'dag_id':'icis-rater-cdr-batch-0.4.dev.3.0'
    ...
    authCheck = COMMON.getICISAuthCheckWflow('5c36ba3e3ffe4b45a86a1508aa9b233b')
    ...
    ldinCdrJob = COMMON.getICISKubernetesPodOperator_v1({
        'id' : '7d5ebb3ce456473cbb80320b636619a1',
        'volumes': ldinCdrJob_vol,
        'volume_mounts': ldinCdrJob_volMnt,
        'env_from':ldinCdrJob_env,
        'task_id':'ldinCdrJob',
        'image':'/icis/icis-rater-batch-cdr:0.4.0.19',
        'arguments':["--job.names=ldinCdrJob", "runType=T", "useYm=202409"],
    ...
    cdrCretJob = COMMON.getICISKubernetesPodOperator_v1({
        'id' : 'c9c665cdc6794b0dbba103597c1773a9',
        'volumes': cdrCretJob_vol,
        'volume_mounts': cdrCretJob_volMnt,
        'env_from':cdrCretJob_env,
        'task_id':'cdrCretJob',
        'image':'/icis/icis-rater-batch-cdr:0.4.0.19',
        'arguments':["--job.names=cdrCretJob", "runType=T", "useYm=202409"],
        'taskAlrmStYn': 'N', # 시작 알림 전송
        'taskAlrmFnsYn': 'N' # 종료 알림 전송
    })
    ...
    drotCdrJob = COMMON.getICISKubernetesPodOperator_v1({
        'id' : 'da232cfbd01a42d3a9190f8633507cd0',
        'volumes': drotCdrJob_vol,
        'volume_mounts': drotCdrJob_volMnt,
        'env_from':drotCdrJob_env,
        'task_id':'drotCdrJob',
        'image':'/icis/icis-rater-batch-cdr:0.4.0.19',
        'arguments':["--job.names=drotCdrJob", "runType=T", "useYm=202409"],
        'taskAlrmStYn': 'N', # 시작 알림 전송
        'taskAlrmFnsYn': 'N' # 종료 알림 전송
    })
    ...
    Complete = COMMON.getICISCompleteWflowTask('5c36ba3e3ffe4b45a86a1508aa9b233b')
